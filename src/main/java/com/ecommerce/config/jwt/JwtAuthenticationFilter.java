...
import com.ecommerce.service.user.UserDetailsServiceImpl;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if (excludeEndpoint(request, response, filterChain)) {
            ...
            log.info("ignored the 'http://localhost:8080/api/v1/auth/user/signup' path.");
            return;
        }
    ...
    private boolean excludeEndpoint(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        String excludePath2 = "http://localhost:8080/api/v1/auth/user/signup";
        if (path.startsWith(excludePath) && path.startsWith(excludePath2)) {
            filterChain.doFilter(request, response);
            return true;
        }
        return false;
